...
public class SleepHistoryPage {
    ...
    private void setupUI() {
        ...
        startTimeColumn.setCellValueFactory(cellData -> 
            new SimpleStringProperty(cellData.getValue().getStartTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
        );

        TableColumn<Sleep, String> endTimeColumn = new TableColumn<>("WakeUp Time");
        endTimeColumn.setCellValueFactory(cellData -> 
            new SimpleStringProperty(cellData.getValue().getEndTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
        );
        ...
        moodColumn.setCellValueFactory(cellData -> 
            new SimpleStringProperty(cellData.getValue().getMood())
        );

        TableColumn<Sleep, String> disturbancesColumn = new TableColumn<>("Disturbances");
        disturbancesColumn.setCellValueFactory(cellData -> 
            new SimpleStringProperty(cellData.getValue().getDisturbances())
        );

        TableColumn<Sleep, Integer> sleepQualityColumn = new TableColumn<>("Sleep Quality");
        sleepQualityColumn.setCellValueFactory(cellData -> 
            new SimpleIntegerProperty(cellData.getValue().getQuality()).asObject()
        );

        TableColumn<Sleep, String> sleepStatusColumn = new TableColumn<>("Sleep Status");
        sleepStatusColumn.setCellValueFactory(cellData -> 
            new SimpleStringProperty(calculateSleepStatus(cellData.getValue()))
        );
        ...
        // Populate the table with sleep history data if it's not null
        if (sleepHistory != null) {
            tableView.getItems().setAll(sleepHistory);
        }
        ...
        scene.getStylesheets().add(getClass().getResource("/resources/sleep.css").toExternalForm()); // Add CSS file
